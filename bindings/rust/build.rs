fn main() {
    ...
    c_config.compile("tree-sitter-xresources");

    println!("cargo:rustc-check-cfg=cfg(with_highlights_query)");
    if !"queries/xresources/highlights.scm".is_empty() && std::path::Path::new("queries/xresources/highlights.scm").exists() {
        println!("cargo:rustc-cfg=with_highlights_query");
    }
    println!("cargo:rustc-check-cfg=cfg(with_injections_query)");
    if !"queries/xresources/injections.scm".is_empty() && std::path::Path::new("queries/xresources/injections.scm").exists() {
